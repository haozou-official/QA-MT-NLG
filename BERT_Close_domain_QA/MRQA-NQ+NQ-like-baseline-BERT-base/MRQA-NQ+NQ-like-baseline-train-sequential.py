...
'''
#nqlike_train_df = pd.read_json(nqlike_train_path, lines=True, orient='records')
#nq_train_df = pd.read_json("/fs/clip-quiz/saptab1/QA-MT-NLG/Datasets/NaturalQuestions_train_reformatted.json", lines=True, orient='records')
#nq_and_nqlike_df = pd.concat(( nq_train_df, nqlike_train_df), axis=0, ignore_index=True)

#print('Total Training Size = ', len(nq_and_nqlike_df))

#nq_and_nqlike_df.to_json('/fs/clip-quiz/saptab1/QA-MT-NLG/Datasets/nq_and_nqlike_processed.json', lines=True, orient='records', force_ascii=False)

#print('Dataset Formatted and Concatenated!')
print('Dataset Formatted!')

print('Loading validation data...')
tiny_val_data = load_dataset('json', data_files='../Datasets/NaturalQuestions_dev_reformatted.json', split='train[:]')
...
tiny_nq_train_data = load_dataset('json', data_files='/fs/clip-quiz/saptab1/QA-MT-NLG/Datasets/NaturalQuestions_train_reformatted.json', split='train[:]')
...
checkpoint_dir = "Tryout_Oct24_MRQA_NQ_NQlike_Closed_Domain_QA_BERTbase_seq"

# optimizer
nq_learning_rate=2e-5
nqlike_learning_rate=2.85e-6
num_train_epochs=3
weight_decay=0.01
checkpoint_periodicity = 2500
EPOCHS = 3

# Set Up Different Optimizers for NQ and NQ like with different learning rates for weighing issue
#opt1 = torch.optim.Adam(model.parameters(), lr = nq_learning_rate, weight_decay = weight_decay)

#opt2 = torch.optim.Adam(model.parameters(), lr = nqlike_learning_rate, weight_decay = weight_decay)
...
for epoch in range(1, EPOCHS+1):
   ...
   for i,batch in enumerate(nq_trainloader):
      ...
      if (i+1)%checkpoint_periodicity == 0:
