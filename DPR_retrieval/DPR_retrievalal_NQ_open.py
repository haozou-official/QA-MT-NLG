...
nq_questions_train = []
nq_answers_train = []
with jsonlines.open('./Datasets/NQ-open.train-train.jsonl') as f:
    for line in f.iter():
        nq_questions_train.append(line['question'])
        nq_answers_train.append(line['answer'][0])
print("Done reading questions")

for i in range(len(nq_questions_train)):
  if i%1000==0:
    print("progress: ", str(i)+"/"+str(len(nq_questions_train))+"\n")
  question = nq_questions_train[i]
  ...
  retrieved_result = {}
  retrieved_result['question'] = nq_questions_train[i]
  retrieved_result['answer'] = nq_answers_train[i]
...
# save
with open("dpr_retrieval/Nov8_retrieval_NQ.json", 'w') as f:
